...
const getStatTotal = (stat: { base: number, skill: number, career: number, imprint: number, isCore: boolean }) => {
  ...
  if (stat.isCore) {
    ...
    // 🌟 소수점 손실 방지를 위해 내림(floor) 대신 반올림(round) 처리!
    raw = Math.round(raw); 
    
    // 3. 퍼센트 영향을 받지 않는 공통 깡파워 합산 (바인더, 클랜, 커리어렙, 돌파, 시너지고정, 수동고정, 전능 커리어/각인)
  ...
  // 🌟 최종 스탯 표시 시 소수점이 생겼을 경우를 대비하여 다시 한번 반올림 처리
    ...
    <div class="max-w-[1600px] mx-auto">
      <header class="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">소수점 반올림(Round) 처리를 적용하여 인게임 스탯과 1의 오차도 없도록 완벽하게 보정되었습니다.</p>
        </div>
      </header>
                  ...
                  <tfoot class="bg-neutral-100 dark:bg-neutral-700/50">
                    ...
                    <tr>
                      ...
                      <td colspan="4" class="p-4 border-r border-neutral-200 dark:border-neutral-700 font-bold text-neutral-500 dark:text-neutral-400 text-sm text-left">
                        <div><span class="text-sky-600 font-bold">[% 적용 버프 파워 합산]</span> 강화 +{{ totalPower.autoEnhanceFixed }} / 레벨,도감,팀 등 +{{ totalPower.percentableGrowthBuffSum }}</div>
