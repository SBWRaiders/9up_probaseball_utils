...
const addAutoLineupDgn = (p: PreparedPlayer) => {
  if (autoLineupSelectedDgnIds.value.length >= 4) {
      alert('디그니티 카드는 최대 4장까지만 선택할 수 있습니다.');
      return;
  }
  ...
  setTimeout(() => {
    try {
      ...
      synergys.value.forEach(s => {
          ...
          if (tiers.length > 0) {
              tiers.sort((a, b) => a.req - b.req);
              synergyMap[name] = { type, tiers };
          }
      });

      // 🌟 2. 산해님표 렉 제로 필터링 (DGN VIP 통제 & 벤치 풀 엄격 압축)
      ...
      // 🚨 벤치 서치 풀은 무조건 SEA, HIT, ACE, TEA로 한정하여 경우의 수 폭발(렉) 원천 차단!
      ...
      // 가상 견적 시스템을 위한 '벤치 창고(Warehouse)' 현황 파악
      const warehouseTags: Record<string, { B: number, P: number }> = {};
      benchCandidates.forEach(p => {
          const isPit = isPitcher(p);
          if (p._tags) {
              p._tags.forEach((t: string) => {
                  if (!warehouseTags[t]) warehouseTags[t] = { B: 0, P: 0 };
                  if (isPit) warehouseTags[t].P++; else warehouseTags[t].B++;
              });
          }
      });
      ...
      const calculateBaseStats = (p: any) => {
         const coreSum = isPitcher(p) ? (safeNum(p.movement)+safeNum(p.longHitSuppression)+safeNum(p.homeRunSuppression)+safeNum(p.control)+safeNum(p.stuff)) : (safeNum(p.contact)+safeNum(p.gapPower)+safeNum(p.homeRunPower)+safeNum(p.plateDiscipline)+safeNum(p.strikeoutAvoidance));
         const nonCoreSum = isPitcher(p) ? (safeNum(p.defense)+safeNum(p.pitchLimit)+safeNum(p.runnerControl)) : (safeNum(p.defense)+safeNum(p.stealing)+safeNum(p.baseRunning));
      ...
      const evaluateLineupScore = (tempLineup: Record<string, any>) => {
          ...
          for(const p of players) {
             ...
             const pureName = getPureName(p.name);
             const role = isPitcher(p) ? 'P' : 'B';
             const key = pureName + '_' + role;
             
             if(nameSet.has(key)) return -2000000; // 동명이인 원천 차단
             nameSet.add(key);
          ...
          const synCounts: Record<string, { B: number, P: number, mainB: number, mainP: number }> = {};
          players.forEach(p => {
             const isPit = isPitcher(p);
             const isMain = mainSlots.includes(p._slot);
             if(p._tags) {
                 p._tags.forEach((tag: string) => {
                    if(!synCounts[tag]) synCounts[tag] = { B: 0, P: 0, mainB: 0, mainP: 0 };
                    if(isPit) { synCounts[tag].P++; if(isMain) synCounts[tag].mainP++; } 
                    else { synCounts[tag].B++; if(isMain) synCounts[tag].mainB++; }
                 });
             }
          });

          globalBuffs.synergyMasteries.forEach((m: string) => {
             if(m && synCounts[m]) { 
                 synCounts[m].B++; synCounts[m].P++; 
                 synCounts[m].mainB++; synCounts[m].mainP++; 
             }
          });
          ...
          Object.keys(synCounts).forEach(tag => {
              ...
              const calcPot = (count: number, mainCount: number, availInWarehouse: number, isValidType: boolean) => {
                  if (!isValidType || mainCount === 0 || count === 0) return 0; // 주전에 수혜자가 없으면 즉시 0점 (허세 컷트)
                  ...
                  for (let t of smap.tiers) {
                      if (count >= t.req) {
                          ...
                          score += pwrVal * mainCount; // 1% = 20점 환율 * 수혜자 수
                          isFullyActive = true;
                      } else {
                  ...
                  // 아직 불이 안 켜졌다면(빌드업 구간), 창고 재고를 확인해 가상 견적 산출
                  if (nextReq > 0 && !isFullyActive) {
                      ...
                      if (maxPossible >= nextReq) {
                          // 창고 털면 켤 수 있다! ➔ 90% 가불 (N빵 비율 곱해서 빌드업 장려)
                          score += totalExpected * 0.9 * (count / nextReq); 
                      } else {
                          // 창고 다 털어도 못 켬 ➔ 희망 고문 50% 가불
              ...
              potentialScore += calcPot(synCounts[tag].B, synCounts[tag].mainB, warehouseTags[tag]?.B || 0, smap.type === 'batter' || smap.type === 'both');
              potentialScore += calcPot(synCounts[tag].P, synCounts[tag].mainP, warehouseTags[tag]?.P || 0, smap.type === 'pitcher' || smap.type === 'both');
          });
          ...
          for(const slot of Object.keys(tempLineup)) {
             ...
             if(slot.startsWith('BENCH')) {
                 teamScore += (p._rawStats || 0) * 0.0000001; // 벤치는 깡스탯 무시, 철저히 시너지 점수로만 평가
                 continue; 
             }
             ...
             if (p._tags) {
                 p._tags.forEach((tag: string) => {
                     ...
                     smap.tiers.forEach(t => { 
                         if (count >= t.req) { pwr = t.power; unit = t.unit; }
                     });
                     
                     if (pwr > 0) {
                         let isAmp = false;
                         globalBuffs.synergyMasteries.forEach((m, i) => { if(m===tag && globalBuffs.amplifiedMasteryIndex===i) isAmp=true; });
                         if(isAmp) pwr += (unit === 'percent') ? 0.5 : 50; 
                         
                         if (unit === 'percent') autoSynergyPercent += pwr;
                         else autoSynergyFixed += pwr;
                     }
                 });
             }
      ...
      while(improved && iterations < MAX_ITER) {
          ...
          for(const slot of mainSlots) {
              ...
              for(const p of mainCandidates) {
                  ...
                  if (oldPlayer && getMappedGrade(oldPlayer.grade) === 'DGN') {
                      ...
                      if (!altSlotForDGN) {
                          if (evictedBenchSlot) curLineup[evictedBenchSlot] = evictedBenchPlayer; // 롤백
                          continue; 
                      }
                  }
                  ...
                  // 테스트 종료 후 롤백 (선택된 경우 나중에 한꺼번에 반영)
          ...
          for(let i=1; i<=8; i++) {
              ...
              for(const p of benchCandidates) {
                  ...
                  const oldPlayer = curLineup[slot];
                  curLineup[slot] = p; // 잉여 강제 퇴거 및 새 후보 투입
                  const newScore = evaluateLineupScore(curLineup);
      ...
      Object.keys(curLineup).forEach(k => {
         if(curLineup[k]) {
             ...
             if (playerBuffs.value[k]) {
                 ...
                 playerBuffs.value[k].selectedSkills = [];
                 playerBuffs.value[k].enhancementLevel = getMappedGrade(curLineup[k].grade) === 'DGN' ? 10 : 15;
                 playerBuffs.value[k].careers = Array(6).fill(null).map(() => ({ grade: '마스터', statType: '전체 능력치', value: getMappedGrade(curLineup[k].grade) === 'DGN' ? 46 : 60 }));
             }
         }
      });
...
// ========================================================
const isAdmin = ref(localStorage.getItem('9up_admin_unlocked_v3') === 'true');
...
const handleTitleClick = () => {
  ...
  if (titleClickCount >= 5) {
    ...
    if (isAdmin.value) {
      isAdmin.value = false;
      localStorage.removeItem('9up_admin_unlocked_v3');
      alert('관리자 모드 비활성화: AI 기능을 숨깁니다. 🔒');
    } else {
      ...
      if (pwd === secret) {
        isAdmin.value = true;
        localStorage.setItem('9up_admin_unlocked_v3', 'true');
        alert('관리자 모드 활성화: AI 자동 편성 기능이 켜졌습니다! 🚀');
      } else if (pwd !== null) {
        alert('비밀번호가 틀렸습니다.');
      }
    }
      ...
      <div class="p-5 flex flex-col gap-5 overflow-y-auto">
        ...
        <div class="bg-amber-50 dark:bg-amber-900/20 p-3 rounded-xl border border-amber-200 dark:border-amber-800/50">
          <p class="text-[11px] text-amber-800 dark:text-amber-300 font-bold leading-relaxed">
            ※ 현재 덱이 지워지고, <strong>{{ autoLineupTeam.name }} 최고 시너지 주전 20명 + 팀플 벤치 + 시너지 토템(SEA) 벤치</strong>로 로스터가 1초 만에 자동 완성됩니다!
          </p>
        </div>
      </div>
