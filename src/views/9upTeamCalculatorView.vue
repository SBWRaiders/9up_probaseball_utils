          ...
          <div class="flex-1 overflow-y-auto p-1.5 sm:p-2 custom-scrollbar">
            ...
            <div v-else-if="selectedSlot && lineup[selectedSlot] && playerBuffs[selectedSlot]" class="space-y-3 animate-in fade-in flex flex-col h-full">
              ...
              <div class="flex items-center gap-3 p-2 bg-neutral-100 dark:bg-neutral-700/50 rounded-xl flex-shrink-0 border border-neutral-200">
                <img :src="getGradeImage(lineup[selectedSlot].grade)" class="w-10 h-10 object-contain drop-shadow" @error="hideImage"/>
                <div>
              ...
              <!-- 🌟 탭 버튼 삭제됨! 아래부터 하나의 화면으로 쭈욱 이어집니다 🌟 -->
              <div class="flex-1 overflow-y-auto custom-scrollbar pr-1 pb-2 space-y-3">
                
                <!-- 🌟 1. 최종 세부 능력치 뷰어 (수동입력 삭제, 결과만 4칸 2줄로 깔끔하게 배치) -->
                <div v-if="computedPlayerStats[selectedSlot]" class="bg-indigo-50 dark:bg-indigo-900/10 p-2.5 rounded-xl border border-indigo-100 shadow-sm flex-shrink-0">
                  <h3 class="text-[10px] font-bold text-indigo-800 dark:text-indigo-300 mb-2 flex items-center gap-1"><TrendingUp class="w-3 h-3"/> 최종 세부 능력치</h3>
                  ...
                  <div class="grid grid-cols-4 gap-1.5">
                     <div v-for="stat in (isPitcher(lineup[selectedSlot]) ? pitcherStats : batterStats)" :key="stat" class="flex flex-col items-center justify-center py-1.5 border border-indigo-100 rounded-lg bg-white shadow-sm flex-shrink-0 gap-0.5">
                        <span class="text-[9px] font-bold text-neutral-500">{{ STAT_LABELS[stat] || stat }}</span>
                        <span class="text-[12px] font-black text-indigo-700">{{ computedPlayerStats[selectedSlot].stats[stat] }}</span>
                     </div>
                  </div>
                </div>
                ...
                <div v-if="!isPitcher(lineup[selectedSlot])" class="bg-orange-50 p-2.5 rounded-xl border border-orange-100 flex-shrink-0">
                  ...
                  <select v-model.number="playerBuffs[selectedSlot].battingOrder" class="w-full py-1 px-2 rounded-lg border border-orange-200 bg-white text-[11px] outline-none">
                ...
                <!-- 🌟 7. 각인 장착 슬롯 -->
                <div class="bg-white p-2.5 rounded-xl border border-neutral-200 shadow-sm flex-shrink-0">
                  <div class="flex justify-between items-center mb-1.5">
