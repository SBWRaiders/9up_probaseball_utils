...
<!-- 중앙: 라인업 보드 -->
        <section class="lg:col-span-5 flex flex-col rounded-2xl bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 min-h-0 shadow-sm overflow-hidden relative">
          ...
          <div class="flex-1 overflow-hidden p-1 sm:p-2 bg-neutral-50/30 dark:bg-neutral-900/30 flex flex-col items-center">
            
            <!-- 타자 다이아몬드 UI -->
            <div v-if="lineupViewMode === 'batter'" class="w-full h-full flex flex-col justify-between items-center py-1">
               
               <!-- 외야수 (3명) -->
               <div class="flex justify-center gap-1 sm:gap-1.5 w-full h-[32%] min-h-0">
                 <div v-for="pos in ['LF', 'CF', 'RF']" :key="pos" @dragover.prevent @drop="onDrop($event, pos)" class="flex-1 min-w-0 flex justify-center items-center h-full">
                   <div v-if="!lineup[pos]" class="h-full w-auto max-w-full aspect-[5/7] border border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-neutral-300 dark:border-neutral-600 bg-neutral-50/50 dark:bg-neutral-800/30 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 text-neutral-400" :class="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30': selectedSlot === pos}" @click="selectSlot(pos)"><span class="text-[11px] font-bold">{{ pos }}</span></div>
                   <div v-else draggable="true" @dragstart="onDragStart($event, pos)" class="relative h-full w-auto max-w-full aspect-[5/7] border rounded-xl flex flex-col items-center p-0 cursor-pointer transition-all shadow-sm group overflow-hidden bg-neutral-100 dark:bg-neutral-800" :class="{'border-indigo-500 ring-2 ring-indigo-400': selectedSlot === pos, 'border-neutral-200 dark:border-neutral-600': selectedSlot !== pos}" @click="selectSlot(pos)">
                      <div class="absolute top-1 left-1.5 text-[10px] font-black text-white drop-shadow-[0_1px_3px_rgba(0,0,0,1)] z-10">{{ pos }}</div>
                      ...
                      <div v-if="playerBuffs[pos]?.battingOrder && !isPitcher(lineup[pos])" class="absolute top-1 left-1/2 -translate-x-1/2 text-[9px] font-bold bg-orange-600 text-white px-1.5 py-0.5 rounded shadow-md z-10">{{ playerBuffs[pos].battingOrder }}번</div>
                      
                      <img :src="getPlayerImage(lineup[pos])" class="w-full h-full object-cover" @error="hideImage" />
                      
                      <div class="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-black/95 via-black/50 to-transparent flex flex-col justify-end items-center pb-1.5 px-1 pointer-events-none">
                         <div class="text-[10px] sm:text-[11px] font-bold text-white w-full text-center truncate drop-shadow-md leading-tight">{{ lineup[pos].name }}</div>
                         <div class="text-[12px] sm:text-[13px] font-black text-amber-400 tracking-tight drop-shadow-md leading-tight mt-0.5">{{ calculatePlayerPower(lineup[pos], pos).toLocaleString() }}</div>
                      </div>
                   </div>
                 </div>
               </div>
               ...
               <!-- 내야수 (4명) -->
               <div class="flex justify-center gap-1 sm:gap-1.5 w-full h-[32%] min-h-0">
                 <div v-for="pos in ['3B', 'SS', '2B', '1B']" :key="pos" @dragover.prevent @drop="onDrop($event, pos)" class="flex-1 min-w-0 flex justify-center items-center h-full">
                   <div v-if="!lineup[pos]" class="h-full w-auto max-w-full aspect-[5/7] border border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-neutral-300 dark:border-neutral-600 bg-neutral-50/50 dark:bg-neutral-800/30 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 text-neutral-400" :class="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30': selectedSlot === pos}" @click="selectSlot(pos)"><span class="text-[11px] font-bold">{{ pos }}</span></div>
                   <div v-else draggable="true" @dragstart="onDragStart($event, pos)" class="relative h-full w-auto max-w-full aspect-[5/7] border rounded-xl flex flex-col items-center p-0 cursor-pointer transition-all shadow-sm group overflow-hidden bg-neutral-100 dark:bg-neutral-800" :class="{'border-indigo-500 ring-2 ring-indigo-400': selectedSlot === pos, 'border-neutral-200 dark:border-neutral-600': selectedSlot !== pos}" @click="selectSlot(pos)">
                      <div class="absolute top-1 left-1.5 text-[10px] font-black text-white drop-shadow-[0_1px_3px_rgba(0,0,0,1)] z-10">{{ pos }}</div>
                      <button class="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/50 text-white hover:bg-red-500 flex items-center justify-center text-[12px] opacity-0 group-hover:opacity-100 transition-opacity z-20 backdrop-blur-sm" @click.stop="clearSlot(pos)">×</button>
               ...
               <!-- 포수 & 지명타자 -->
               <div class="flex justify-center gap-4 sm:gap-8 w-full h-[32%] min-h-0">
                 <div v-for="pos in ['C', 'DH']" :key="pos" @dragover.prevent @drop="onDrop($event, pos)" class="flex-1 min-w-0 flex justify-center items-center h-full max-w-[280px]">
                   <div v-if="!lineup[pos]" class="h-full w-auto max-w-full aspect-[5/7] border border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-neutral-300 dark:border-neutral-600 bg-neutral-50/50 dark:bg-neutral-800/30 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 text-neutral-400" :class="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30': selectedSlot === pos}" @click="selectSlot(pos)"><span class="text-[11px] font-bold">{{ pos }}</span></div>
                   <div v-else draggable="true" @dragstart="onDragStart($event, pos)" class="relative h-full w-auto max-w-full aspect-[5/7] border rounded-xl flex flex-col items-center p-0 cursor-pointer transition-all shadow-sm group overflow-hidden bg-neutral-100 dark:bg-neutral-800" :class="{'border-indigo-500 ring-2 ring-indigo-400': selectedSlot === pos, 'border-neutral-200 dark:border-neutral-600': selectedSlot !== pos}" @click="selectSlot(pos)">
                      <div class="absolute top-1 left-1.5 text-[10px] font-black text-white drop-shadow-[0_1px_3px_rgba(0,0,0,1)] z-10">{{ pos }}</div>
                      <button class="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/50 text-white hover:bg-red-500 flex items-center justify-center text-[12px] opacity-0 group-hover:opacity-100 transition-opacity z-20 backdrop-blur-sm" @click.stop="clearSlot(pos)">×</button>
            ...
            <!-- 투수 UI -->
            <div v-else-if="lineupViewMode === 'pitcher'" class="w-full h-full flex flex-col justify-between items-center py-1">
              <div class="w-full h-[48%] flex flex-col items-center min-h-0">
                <h3 class="text-[11px] font-bold text-neutral-500 mb-1 shrink-0">선발 투수</h3>
                <div class="flex justify-center gap-1 sm:gap-1.5 w-full h-full min-h-0 pb-1">
                  <div v-for="i in 5" :key="'SP'+i" @dragover.prevent @drop="onDrop($event, 'SP'+i)" class="flex-1 min-w-0 flex justify-center items-center h-full">
                     <div v-if="!lineup['SP'+i]" class="h-full w-auto max-w-full aspect-[5/7] border border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-neutral-300 dark:border-neutral-600 bg-neutral-50/50 dark:bg-neutral-800/30 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 text-neutral-400" :class="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30': selectedSlot === 'SP'+i}" @click="selectSlot('SP'+i)"><span class="text-[11px] font-bold">{{ 'SP'+i }}</span></div>
                   <div v-else draggable="true" @dragstart="onDragStart($event, 'SP'+i)" class="relative h-full w-auto max-w-full aspect-[5/7] border rounded-xl flex flex-col items-center p-0 cursor-pointer transition-all shadow-sm group overflow-hidden bg-neutral-100 dark:bg-neutral-800" :class="{'border-indigo-500 ring-2 ring-indigo-400': selectedSlot === 'SP'+i, 'border-neutral-200 dark:border-neutral-600': selectedSlot !== 'SP'+i}" @click="selectSlot('SP'+i)">
                      <div class="absolute top-1 left-1.5 text-[10px] font-black text-white drop-shadow-[0_1px_3px_rgba(0,0,0,1)] z-10">{{ 'SP'+i }}</div>
                      ...
                      <img :src="getPlayerImage(lineup['SP'+i])" class="w-full h-full object-cover" @error="hideImage" />
                      
                      <div class="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-black/95 via-black/50 to-transparent flex flex-col justify-end items-center pb-1.5 px-1 pointer-events-none">
                         <div class="text-[10px] sm:text-[11px] font-bold text-white w-full text-center truncate drop-shadow-md leading-tight">{{ lineup['SP'+i].name }}</div>
                         <div class="text-[12px] sm:text-[13px] font-black text-amber-400 tracking-tight drop-shadow-md leading-tight mt-0.5">{{ calculatePlayerPower(lineup['SP'+i], 'SP'+i).toLocaleString() }}</div>
                      </div>
                   </div>
                  </div>
                </div>
              ...
              <div class="w-full h-[48%] flex flex-col items-center min-h-0">
                <h3 class="text-[11px] font-bold text-neutral-500 mb-1 shrink-0">계투 및 마무리</h3>
                <div class="flex justify-center gap-1 sm:gap-1.5 w-full h-full min-h-0 pb-1">
                  <div v-for="i in 6" :key="'RP'+i" @dragover.prevent @drop="onDrop($event, 'RP'+i)" class="flex-1 min-w-0 flex justify-center items-center h-full">
                     <div v-if="!lineup['RP'+i]" class="h-full w-auto max-w-full aspect-[5/7] border border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-neutral-300 dark:border-neutral-600 bg-neutral-50/50 dark:bg-neutral-800/30 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 text-neutral-400" :class="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30': selectedSlot === 'RP'+i}" @click="selectSlot('RP'+i)"><span class="text-[11px] font-bold">{{ 'RP'+i }}</span></div>
                   <div v-else draggable="true" @dragstart="onDragStart($event, 'RP'+i)" class="relative h-full w-auto max-w-full aspect-[5/7] border rounded-xl flex flex-col items-center p-0 cursor-pointer transition-all shadow-sm group overflow-hidden bg-neutral-100 dark:bg-neutral-800" :class="{'border-indigo-500 ring-2 ring-indigo-400': selectedSlot === 'RP'+i, 'border-neutral-200 dark:border-neutral-600': selectedSlot !== 'RP'+i}" @click="selectSlot('RP'+i)">
                      <div class="absolute top-1 left-1.5 text-[10px] font-black text-white drop-shadow-[0_1px_3px_rgba(0,0,0,1)] z-10">{{ 'RP'+i }}</div>
                      ...
                      <img :src="getPlayerImage(lineup['RP'+i])" class="w-full h-full object-cover" @error="hideImage" />
                      
                      <div class="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-black/95 via-black/50 to-transparent flex flex-col justify-end items-center pb-1.5 px-1 pointer-events-none">
                         <div class="text-[10px] sm:text-[11px] font-bold text-white w-full text-center truncate drop-shadow-md leading-tight">{{ lineup['RP'+i].name }}</div>
                         <div class="text-[12px] sm:text-[13px] font-black text-amber-400 tracking-tight drop-shadow-md leading-tight mt-0.5">{{ calculatePlayerPower(lineup['RP'+i], 'RP'+i).toLocaleString() }}</div>
                      </div>
                   </div>
                  </div>
                </div>
            ...
            <!-- 벤치 UI -->
            <div v-else class="w-full h-full flex flex-col justify-between items-center py-1">
               <h3 class="text-[11px] font-bold text-neutral-500 mb-1 shrink-0">벤치 멤버</h3>
               <div class="w-full h-full flex flex-col justify-between min-h-0 pb-1">
                 <!-- 벤치 윗줄 4명 -->
                 <div class="flex justify-center gap-1 sm:gap-1.5 w-full h-[48%] min-h-0">
                    <div v-for="i in 4" :key="'BENCH'+i" @dragover.prevent @drop="onDrop($event, 'BENCH'+i)" class="flex-1 min-w-0 flex justify-center items-center h-full">
                       <div v-if="!lineup['BENCH'+i]" class="h-full w-auto max-w-full aspect-[5/7] border border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-neutral-300 dark:border-neutral-600 bg-neutral-50/50 dark:bg-neutral-800/30 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 text-neutral-400" :class="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30': selectedSlot === 'BENCH'+i}" @click="selectSlot('BENCH'+i)"><span class="text-[11px] font-bold">{{ 'BENCH'+i }}</span></div>
                     <div v-else draggable="true" @dragstart="onDragStart($event, 'BENCH'+i)" class="relative h-full w-auto max-w-full aspect-[5/7] border rounded-xl flex flex-col items-center p-0 cursor-pointer transition-all shadow-sm group overflow-hidden bg-neutral-100 dark:bg-neutral-800" :class="{'border-indigo-500 ring-2 ring-indigo-400': selectedSlot === 'BENCH'+i, 'border-neutral-200 dark:border-neutral-600': selectedSlot === 'BENCH'+i}" @click="selectSlot('BENCH'+i)">
                        <div class="absolute top-1 left-1.5 text-[10px] font-black text-white drop-shadow-[0_1px_3px_rgba(0,0,0,1)] z-10">{{ 'BENCH'+i }}</div>
                        <button class="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/50 text-white hover:bg-red-500 flex items-center justify-center text-[12px] opacity-0 group-hover:opacity-100 transition-opacity z-20 backdrop-blur-sm" @click.stop="clearSlot('BENCH'+i)">×</button>
                        <div v-if="playerBuffs['BENCH'+i]?.battingOrder && !isPitcher(lineup['BENCH'+i])" class="absolute top-1 left-1/2 -translate-x-1/2 text-[9px] font-bold bg-orange-600 text-white px-1.5 py-0.5 rounded shadow-md z-10">{{ playerBuffs['BENCH'+i].battingOrder }}번</div>
                        
                        <img :src="getPlayerImage(lineup['BENCH'+i])" class="w-full h-full object-cover" @error="hideImage" />
                        
                        <div class="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-black/95 via-black/50 to-transparent flex flex-col justify-end items-center pb-1.5 px-1 pointer-events-none">
                           <div class="text-[10px] sm:text-[11px] font-bold text-white w-full text-center truncate drop-shadow-md leading-tight">{{ lineup['BENCH'+i].name }}</div>
                           <div class="text-[12px] sm:text-[13px] font-black text-neutral-300 tracking-tight drop-shadow-md leading-tight mt-0.5">{{ calculatePlayerPower(lineup['BENCH'+i], 'BENCH'+i).toLocaleString() }}</div>
                        </div>
                     </div>
                    </div>
                 </div>
                 ...
                 <!-- 벤치 아랫줄 4명 -->
                 <div class="flex justify-center gap-1 sm:gap-1.5 w-full h-[48%] min-h-0">
                    <div v-for="i in 4" :key="'BENCH'+(i+4)" @dragover.prevent @drop="onDrop($event, 'BENCH'+(i+4))" class="flex-1 min-w-0 flex justify-center items-center h-full">
                       <div v-if="!lineup['BENCH'+(i+4)]" class="h-full w-auto max-w-full aspect-[5/7] border border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-neutral-300 dark:border-neutral-600 bg-neutral-50/50 dark:bg-neutral-800/30 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 text-neutral-400" :class="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30': selectedSlot === 'BENCH'+(i+4)}" @click="selectSlot('BENCH'+(i+4))"><span class="text-[11px] font-bold">{{ 'BENCH'+(i+4) }}</span></div>
                     <div v-else draggable="true" @dragstart="onDragStart($event, 'BENCH'+(i+4))" class="relative h-full w-auto max-w-full aspect-[5/7] border rounded-xl flex flex-col items-center p-0 cursor-pointer transition-all shadow-sm group overflow-hidden bg-neutral-100 dark:bg-neutral-800" :class="{'border-indigo-500 ring-2 ring-indigo-400': selectedSlot === 'BENCH'+(i+4), 'border-neutral-200 dark:border-neutral-600': selectedSlot !== 'BENCH'+(i+4)}" @click="selectSlot('BENCH'+(i+4))">
                        <div class="absolute top-1 left-1.5 text-[10px] font-black text-white drop-shadow-[0_1px_3px_rgba(0,0,0,1)] z-10">{{ 'BENCH'+(i+4) }}</div>
                        <button class="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/50 text-white hover:bg-red-500 flex items-center justify-center text-[12px] opacity-0 group-hover:opacity-100 transition-opacity z-20 backdrop-blur-sm" @click.stop="clearSlot('BENCH'+(i+4))">×</button>
                        <div v-if="playerBuffs['BENCH'+(i+4)]?.battingOrder && !isPitcher(lineup['BENCH'+(i+4)])" class="absolute top-1 left-1/2 -translate-x-1/2 text-[9px] font-bold bg-orange-600 text-white px-1.5 py-0.5 rounded shadow-md z-10">{{ playerBuffs['BENCH'+(i+4)].battingOrder }}번</div>
                        
                        <img :src="getPlayerImage(lineup['BENCH'+(i+4)])" class="w-full h-full object-cover" @error="hideImage" />
                        
                        <div class="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-black/95 via-black/50 to-transparent flex flex-col justify-end items-center pb-1.5 px-1 pointer-events-none">
                           <div class="text-[10px] sm:text-[11px] font-bold text-white w-full text-center truncate drop-shadow-md leading-tight">{{ lineup['BENCH'+(i+4)].name }}</div>
                           <div class="text-[12px] sm:text-[13px] font-black text-neutral-300 tracking-tight drop-shadow-md leading-tight mt-0.5">{{ calculatePlayerPower(lineup['BENCH'+(i+4)], 'BENCH'+(i+4)).toLocaleString() }}</div>
                        </div>
                     </div>
                    </div>
                 </div>
